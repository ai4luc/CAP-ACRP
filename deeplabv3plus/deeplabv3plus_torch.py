...
def normalize0to1(imgs):
    ...
    for img in imgs:
        ...
        # Saving as array
        cropped_raster = np.array(cropped_raster).astype('float32')
...
def load_patches(path):
    ...
    for name in image_names_split:
        with rio.open(name) as raster:
            ...
            for i in range(1, 4 + 1):
                band = raster.read(i)
                cropped_raster = normalize0to1(band)
    ...
    # To tensor
    imgtensor_patches = torch.tensor(image_patches, dtype=torch.float32)
    labeltensor_patches = torch.tensor(label_patches, dtype=torch.int32)

    return imgtensor_patches, labeltensor_patches
...
if __name__ == '__main__':
    main(epochs=EPOCHS, batch_size=BATCH_SIZE, num_classes=NUM_CLASSES, path=cerradatav3,
         dir_report=exp_directory, num_channels=4, name_dataset='cerradatav31')
